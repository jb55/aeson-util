...
Transform `Value`s by accumulating an `Applicative` result from every value.
...
Traverse singleton `Value`s, object properties and array `Value`s, accumulating
an `Applicative` result along the way.
